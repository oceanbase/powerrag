...
async def apply_meta_data_filter(
    ...
    from rag.prompts.generator import filter_langextract_docs
    ...
    if method == "auto":
        filters: dict = await gen_meta_filter(chat_mdl, metas, question, meta_data_filter=meta_data_filter, kb_ids=kb_ids)

        regular_filters = filters.get("conditions", []) or []
        logic = filters.get("logic", meta_data_filter.get("logic", "and"))
        langextract_filters = filters.get("langextract_conditions", []) or []

        regular_metas = {k: v for k, v in metas.items() if k != "langextract"}
        regular_ids = meta_filter(regular_metas, regular_filters, logic) if regular_filters else []

        if regular_filters and not regular_ids and langextract_filters:
            ids = await filter_langextract_docs(metas, langextract_filters, None)
        elif langextract_filters:
            ids = await filter_langextract_docs(metas, langextract_filters, regular_ids if regular_ids else None)
        else:
            ids = regular_ids

        if base_doc_ids:
            ids = [d for d in ids if d in set(base_doc_ids)]
        if not ids:
            return None
        return ids
        ...
        if selected_keys:
            ...
            if filtered_metas:
                ...
                doc_ids.extend(meta_filter(metas, filters.get("conditions", []), filters.get("logic", "and")))
                if not doc_ids:
                    return None
    elif method == "manual":
        filters = meta_data_filter.get("manual", []) or []
        if manual_value_resolver:
            filters = [manual_value_resolver(flt) for flt in filters]

        langextract_filters = [
            f for f in filters
            if f.get("key") in {"extraction_class", "extraction_text"} or str(f.get("key", "")).startswith("attributes_")
        ]
        regular_filters = [f for f in filters if f not in langextract_filters]

        regular_metas = {k: v for k, v in metas.items() if k != "langextract"}
        regular_ids = meta_filter(regular_metas, regular_filters, meta_data_filter.get("logic", "and")) if regular_filters else []
